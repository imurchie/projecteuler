...
# make it into FixNums
grid = grid.map do |arr|
  arr.map { |e| e.to_i }
end
...
# I want a general solution, for the fun of it.
x_len = grid.length
y_len = grid[0].length
...
(0...x_len).each do |i|
  (0...y_len).each do |j|
    ...
    if i + 3 < x_len
      # we can have a horizontal run of four, so get the product
      prod = grid[i][j, 4].inject(:*)
      highest = prod if prod > highest
      
      # we might also have an upward horizontal
      if j - 3 >= 0
        prod = element * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3]
        highest = prod if prod > highest
      end
      
      # or a downward horizontal
      if j + 3 < y_len
        prod = element * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3]
        highest = prod if prod > highest
      end
    ...
    # no need to go backwards, since that will be caught by earlier forward traversals
    
    if j + 3 < y_len
      # we can have a vertical run, below
      prod = element * grid[i][j+1] * grid[i][j+2] * grid[i][j+3]
      highest = prod if prod > highest
    end
  end
end
